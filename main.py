...
def run_single_analysis(ts_code: str, start_date: str, end_date: str, 
    ...
    try:
        ...
        logger.info("获取基本面数据...")
        try:
            fundamental_data = fundamental.get_fundamental_data(ts_code)
            if fundamental_data:
                logger.info(f"市盈率 (TTM): {fundamental_data.get('pe_ttm', 'N/A')}")
                logger.info(f"市净率：{fundamental_data.get('pb', 'N/A')}")
                logger.info(f"股息率：{fundamental_data.get('dividend_rate', 'N/A')}")
