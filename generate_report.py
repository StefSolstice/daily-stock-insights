...
def generate_intuitive_report(csv_file):
    ...
    current_volume = df['vol'].iloc[0] if 'vol' in df.columns else 0
    avg_volume = df['vol'].mean() if 'vol' in df.columns else 0
    
    # 分析MACD
    if 'macd' in df.columns and not pd.isna(df['macd'].iloc[0]):
        macd_val = df['macd'].iloc[0]
        signal_val = df['signal'].iloc[0]
        
        if macd_val > 0 and signal_val > 0:
            print('🟢 MACD: 金叉向上，短期看涨')
        elif macd_val < 0 and signal_val < 0:
            print('🔴 MACD: 死叉向下，短期看空')
        else:
            print('🟡 MACD: 临界状态，关注方向选择')
    
    # 分析RSI
    if 'rsi' in df.columns and not pd.isna(df['rsi'].iloc[0]):
        rsi_val = df['rsi'].iloc[0]
        if rsi_val > 70:
            print('🔴 RSI: 超买区域，注意回调风险')
        elif rsi_val < 30:
            print('🟢 RSI: 超卖区域，关注反弹机会')
        elif 40 <= rsi_val <= 60:
            print('🟡 RSI: 中性区域，趋势跟随')
        else:
            print('📊 RSI: 正常波动范围')
    ...
    # 分析均线
    if all(col in df.columns for col in ['close_ma5', 'close_ma10', 'close_ma20']):
        ma5_val = df['close_ma5'].dropna().iloc[0] if not df['close_ma5'].dropna().empty else None
        ma10_val = df['close_ma10'].dropna().iloc[0] if not df['close_ma10'].dropna().empty else None
        ma20_val = df['close_ma20'].dropna().iloc[0] if not df['close_ma20'].dropna().empty else None
        
        if ma5_val and ma10_val and ma20_val:
            if current_close > ma5_val > ma10_val > ma20_val:
                print('🟢 均线: 多线上攻，强势格局')
            elif current_close < ma5_val < ma10_val < ma20_val:
                print('🔴 均线: 空头排列，弱势格局')
            else:
                print('🟡 均线: 多空交织，震荡格局')
