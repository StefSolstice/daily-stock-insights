...
def watchlist():
    ...
    if category_id:
        ...
        ''', (current_user.id, category_id)).fetchall()
    else:
        stocks = conn.execute('''
            SELECT w.*, c.name as category_name 
            FROM watchlist w 
    ...
    return render_template('watchlist.html', stocks=stocks, categories=categories, 
...
def add_category():
    ...
    conn = get_db()
    try:
        conn.execute('INSERT INTO categories (user_id, name) VALUES (?, ?)', 
...
def delete_category(category_id):
    ...
    # 同时清空该分类下的股票分类
    conn.execute('UPDATE watchlist SET category_id = NULL WHERE category_id = ? AND user_id = ?', 
                (category_id, current_user.id))
    conn.commit()
    conn.close()
    return jsonify({'success': True})
...
def search_stock():
    ...
    try:
        ...
        pro = ts.pro_api()
        # 搜索股票
        result = pro.stock_basic(exchange='', list_status='L', fields='ts_code,symbol,name,area,industry')
        stocks = result[result['name'].str.contains(keyword, case=False, na=False)]
